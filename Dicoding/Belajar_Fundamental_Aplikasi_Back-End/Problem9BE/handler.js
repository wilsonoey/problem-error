...
async function deleteUser(request, h) {
  try {
    ...
    return new Promise((resolve, reject) => {
      if (verificator.iduser !== parseInt(iduser)) {
        resolve(h.response(
          authorizationerror('User tidak memiliki akses untuk menghapus user lain')
        ).code(403));
      } else if (verificator.iduser === parseInt(iduser)) {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      if (verificator.iduser === undefined) {
        ...
        connection.query(querychecker, verificator.iduser, async(errorchecker, resultschecker) => {
          const userchecker = resultschecker[0];
